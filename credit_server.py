...
def _extract_user_code(text: str) -> str | None:
    ...
    Ho tro cac kieu:
      1) User code 10 ky tu alphanumeric (format chinh cua tool)
         VD: "85CCFCEA2A", "0TEST6682F"
      2) Ma cu kieu VH001, CODE:VH001 (legacy)
      3) Ten normalize thuan chu cai: NGUYENVANA (5-20 chars)
    """
    if not text:
        return None
    upper = text.strip().upper()

    # --- Uu tien 1: User code 10 ky tu alphanumeric (format tool) ---
    # Pattern: chinh xac 10 ky tu [A-Z0-9], co the la toan bo noi dung
    # hoac nam trong chuoi lon hon (ngan cach boi space/dau cau)
    ...
    # Chi match khi noi dung goc KHONG co khoang trang (1 tu duy nhat)
